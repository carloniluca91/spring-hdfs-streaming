...
import it.luca.streaming.core.utils.HDFSUtils;
...
public class HDFSAvroWriter {
    ...
    public <T, A extends SpecificRecord, P> void write(T payload, SourceSpecification<T, A, P> sourceSpecification) {
        ...
        log.info("{} - Found {} partition value(s) for current batch: ({})",
                dataSourceId, partitionValues.size(), Utils.mkString("|", partitionValues));
        ...
        UserGroupInformation.createRemoteUser(hdfsUser).doAs((PrivilegedAction<Object>) () -> {
            try {
                ...
                String tablePath = HDFSUtils.joinPaths(landingRootPath, sourceSpecification.getTableName());
                ...
                for (P partitionValue: partitionValues) {
                    ...
                    String tablePlusPartitionPath = HDFSUtils.joinPaths(tablePath, String.format("%s=%s", sourceSpecification.getPartitionColumn(), partitionValue));
                    ...
                    String fullFilePath = HDFSUtils.joinPaths(tablePlusPartitionPath, String.format("batch_%s.avro", Utils.now("yyyy_MM_dd_HH_mm_ss")));
    ...
    private  <P> void createTableIfNotExists(SourceSpecification<?, ?, P> sourceSpecification) {
        ...
        if (existingTables.stream().anyMatch(s -> s.equalsIgnoreCase(tableName))) {
            ...
            String tableLocation = HDFSUtils.joinPaths(landingRootPath, tableName);
            String avroSchemaLocation = HDFSUtils.joinPaths(schemasRootPath, sourceSpecification.getAvroSchemaFile());
    ...
    private  <P> void updateMetadata(List<P> batchPartitionValues, SourceSpecification<?, ?, P> sourceSpecification) {
        ...
        List<String> newPartitionValues = Utils.toStreamOf(batchPartitionValues, String::valueOf)
        ...
        if (newPartitionValues.isEmpty()) {
            ...
            log.info("{} - All partitions embedded in current batch ({}) exist already",
                    dataSourceId, Utils.mkString("|", batchPartitionValues));
            ...
            log.info("{} - New partitions embedded in current batch are ({}). Need to update table metadata",
                    dataSourceId, Utils.mkString("|", newPartitionValues));
